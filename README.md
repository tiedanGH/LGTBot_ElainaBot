...
| 能力            | 实现                                                                                                                                                                                                                                                                                                  |
|---------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **零配置加载**     | 作为 ElainaBot 插件，路径全部自包含在 `plugins/LGTBot_ElainaBot/`；编译后启动主框架即自动加载，在群里 @ 机器人即可游玩                                                                                                                                                                                                                    |
...
| **消息渲染与图床**   | C++ 端聚合「@玩家 文本 + 图片」并按引擎原始消息排版；`config.yaml` 选定图床时用 markdown 内嵌图片（随主框架 image_hosting 模块动态发现，支持 `any` 自动依次尝试），保留原生 mention 与按钮，留空 / 上传失败回退 `msg_type=7`（排版压平为「文字 + 首图」）                                                                                                                              |
...
| **主动消息额度守护**  | 按 QQ 官方接口限制统计**每群 / 每用户**每日主动消息量（上限可配，默认 1000）；用满后该目标当日自动退回「刷新按钮 + 阻塞等待」被动机制，次日 0 点随日分桶自动恢复；「数据统计」指令可查当前额度用量                                                                                                                                                                                        |
...
| **对局安全保护**    | 「计划重启」维护模式：手动模式暂停创建新游戏而不影响进行中对局与已建房间，自动模式不限制新游戏创建、对局清空后自动重启；重启 / 退出时进行中对局拒绝释放引擎，避免战绩丢失；状态跨热重载保持、真重启后自动恢复                                                                                                                                                                                            |
| **群管中断授权**    | 抢占引擎管理指令 `%中断`：群主 / 群管理员可强制中断本群卡死的对局（插件用已配置的引擎管理员身份代理该条指令并写入审计），**且仅有这一条管理能力** —— 群管执行其他 `%` 管理指令（清除战绩 / 荣誉等）由插件回「未持有**超级**管理员权限」                                                                                                                                                                   |
...
| **自动化 API**   | 面向框架内其他插件的 HTTP 接口：`POST /api/ext/lgtbot/build/compile`（`{"target": "<名称>", "new": bool}`；同步等待并返回用时）与 `.../build/terminate`（强制中断）；独立 token 认证（面板一键复制）。完整增量编译不开放；另有 `POST .../restart`（立即重启）与 `POST .../planned-restart`（`{"enable", "auto", "reason"}` 请求体 —— 开关计划重启维护模式，`auto: true` 时全部对局结束后自动重启） |
...
| 限制                                                  | 影响                               | 当前应对                                                                                                                                          |
|-----------------------------------------------------|----------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------|
| 主动消息需 `msg_id` / `event_id` 引用                      | 倒计时类被动推送可能失败；超量消息被 QQ 拒绝         | 按场景缓存事件上下文（群 5 分钟 / 私信 60 分钟）；倒数第 2 条起挂「🔄 刷新会话」按钮续额度；全量群内可绕过被动配额走真主动消息(详见上「全量群适配」)；私信侧官方已默认允许向好友主动推送，配置 `sandbox_dm_users: ["all"]` 即全员直推不丢弃 |
