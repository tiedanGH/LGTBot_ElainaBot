...
- **消息渲染与图床** —— C++ 端聚合「@玩家 文本 + 图片」并按引擎原始消息排版
  - `config.yaml` 选定图床时用 markdown 内嵌图片，保留原生 mention 与按钮。使用 image_hosting 模块动态发现，支持 `any` 自动依次尝试。留空 / 上传失败回退 `msg_type=7`
...
- **Web 面板拓展页** —— 侧边栏「LGTBot 机器人」单页多标签
  - 仪表盘 / 指标面板 / 配置管理 / 预编译部署 / 引擎编译 / 数据备份 / 消息日志 / 操作审计 / 崩溃转储 / 用户数据
...
- **自动化 API** —— 面向框架内其他插件的 HTTP 接口，独立 token 认证（面板一键复制）
  ...
  - 重启：`POST .../restart`（立即重启）与 `POST .../planned-restart`（`{"enable", "auto", "reason"}` —— 开关计划重启维护模式，`auto: true` 时全部对局结束后自动重启）
...
- [ ] **测试覆盖补齐** —— 给裸奔模块补单测防回归，缺口：`helpers` / `config` / `audit`，以及 `webui/` 各 page 的数据组装与 action 端点
