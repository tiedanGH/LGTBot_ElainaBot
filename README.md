...
| **编译 API**    | 面向框架内其他插件的 HTTP 接口：`POST /api/ext/lgtbot/build/compile`（`{"target": "<名称>"}`，同步等待并返回用时）与 `.../build/terminate`（强制中断）；独立 token 认证（面板一键复制）；编译失败返回 500 + 日志尾巴。完整 / 全量增量编译不开放 |
