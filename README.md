...
| **Web 面板拓展页** | 侧边栏「LGTBot 机器人」单页多标签：仪表盘 / 指标面板 / 配置管理 / 预编译部署 / 引擎编译 / 数据备份 / 消息日志 / 操作审计 / 用户数据；含一键更新（tag + 子模块双重对比）、缓存清理、运行环境自检、在线编辑配置与整进程重启             |
...
| **数据库备份**     | 每日自动 + 手动按钮 + 一键恢复 + 自动轮转保留 7 份；zip 存主框架 `data/backup/lgtbot/`，SQLite 在线备份用 `Connection.backup()` API，引擎运行中也安全                              |
...
| **运维观测**      | **指标面板**（数据统计 / 图床上传成功率 / 崩溃与重启累计 / 配额压力 / 今日主动消息 / 排行榜 / 10 日趋势）+ **操作审计**（编译 / 备份 / 清缓存 / 更新 / 配置 / 重启 / 换绑等状态变更持久化最近 500 条，按类别多选筛选、区分来源） |
