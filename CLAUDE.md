...
| 限制                                                           | 应对                                                                                                      |
|--------------------------------------------------------------|---------------------------------------------------------------------------------------------------------|
| 被动回复按场景限量：群聊同一 `msg_id` 5 条 / 5 分钟，单聊 4 条 / 60 分钟            | 倒数第 2 条起挂「🔄 刷新会话」按钮，最后一条改「⚠️ 最终刷新」；超量阻塞等待 ≤15s（`quota.ref_quota/ref_ttl/refresh_threshold` 按 key 前缀取值） |
| INTERACTION 事件的 `event_id` 独立计一轮配额（按场景 5/4 条）                | 用户点按钮 → 新 event_id → 新一轮额度                                                                              |
