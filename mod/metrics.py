...
计数器(跨 os.execv 重启不丢,文件即真相源):
  ...
  · 配额压力:耗尽次数(TTL 内引用 5 条真用完)/ 刷新等待超时次数(15s 未等到
    新引用强发降级)。挂 callbacks 发送路径 —— quota 模块内部无法区分
    「无事件上下文」与「真耗尽」,且 wait_and_consume 内部重复调 try_consume
    会重计,故不挂 quota.py。
...
def record_quota_exhausted() -> None:
    """被动配额真耗尽:TTL 内引用的 5 条回复真用完(无上下文的推送不算)。"""
    def _m(d: dict) -> None:
