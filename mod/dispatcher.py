...
async def _send_welcome_menu(event) -> None:
    ...
    try:
        ...
        page_logs.log_outgoing(gid or uid, not (event.is_group and gid), '[欢迎菜单]')
    except Exception as e:
        log.warning(f'菜单回复失败: {e}')

# 本插件监听的消息事件类型，适配「全量群」场景
...
def _read_txt_with_default(path: str, default: str, label: str) -> str:
    ...
    · 每次调用都重新打开文件 —— 这就是「热更新」:管理员直接编辑 txt,无需重启进程
    """
    try:
...
async def lgtbot_welcome_menu(event, match):
    ...
    与 lgtbot_dispatch 空消息分支共享 ``_send_welcome_menu``;入口日志在这里打,标签区分文本 / 按钮路径,方便排查。
    state.started=False(引擎崩溃 30s 窗口)时静默不回 —— 菜单按钮指向的命令都依赖引擎,提前发出去也没用。
    """
    if helpers.is_foreign_event(event):
        return
...
async def lgtbot_data_stats(event, match):
    """收到「数据统计」(文本或按钮)→ 输出 lgtbot.db 游戏数据摘要。
...
async def lgtbot_interaction_relay(event, match):
    ...
    if not event.event_id:
        return
    appid_str = event.appid or ''
    # event_id 同样带场景:群里点按钮产生的 event_id 不能用于该用户私信,所以两条分支必须互斥。
...
async def lgtbot_interaction_dispatch(event, match):
    ...
    与 lgtbot_dispatch 的消息处理流程几乎一致,差异仅:
...
def set_planned_mode(enable: bool, reason: str = '', auto: bool = False) -> tuple[bool, str]:
    ...
    ``reason`` 为维护原因,仅在开启时记录并展示给玩家(见 ``_planned_restart_notice``);
    ``auto=True`` 同时启用**自动重启**:watcher 轮询进行中对局,全部结束并静默 30s 后自动执行重启
    (默认手动 —— 管理员自己点重启)。两种模式对玩家的差别:手动模式拦下新建房间回维护提示;
    **自动模式不限制新游戏创建**。关闭时 reason / auto 一并清掉。回执带剩余进行中对局数。
    """
...
def ensure_auto_restart_watcher_on_load() -> None:
    ...
    正常情况下旧 watcher task(引用旧模块代码,读同一份持久 state)跨热重载继续服务,
    _ensure 幂等直接返回;仅在旧 task 因异常终止时真正重建。
    """
